function [] = topoplot_connect_dgz(displayStruct, loc_file, colorfile)
...
% * *ds.cx*(optional) - 1 x 2 matrix specifying minimum
% and maximum values of colormap to display.  --dgz
...
%
if isfield(displayStruct, 'HEADRINGWIDTH')
    HEADRINGWIDTH = displayStruct.HEADRINGWIDTH;
end
...
if isfield(displayStruct, 'EMARKERSIZE')
    EMARKERSIZE = displayStruct.EMARKERSIZE;
end
if isfield(displayStruct, 'add_size')
    add_size = displayStruct.add_size;
else
    add_size = 5;
end
if isfield(displayStruct, 'line_width')
    line_width = displayStruct.line_width;
else
    line_width = 2;
end
if isfield(displayStruct, 'line_alpha')
    line_alpha = displayStruct.line_alpha;
else
    line_alpha = 1;
end
%
...
if isfield(displayStruct, 'EMARKERLINEWIDTH')
    EMARKERLINEWIDTH = displayStruct.EMARKERLINEWIDTH;
end
...
if ~exist( 'colorfile', 'var')
    ...
    cM = colormap(colorfile); % dgz 20240108
end
if isfield(displayStruct, 'cx') % dgz 20240108
    cmax = size(cM, 1);
    if displayStruct.cx(1) == 0
        cM = cM(round(cmax*displayStruct.cx(1))+1:round(cmax*displayStruct.cx(2)), :);
    else
        cM = cM(round(cmax*displayStruct.cx(1)):round(cmax*displayStruct.cx(2)), :);
    end
end
...
tmp_elec_size = ones(1,length(y))*EMARKERSIZE;
for kk = 1:numChanPairs
    plot3(y(displayStruct.chanPairs(kk, :)), x(displayStruct.chanPairs(kk, :)), [ELECTRODE_HEIGHT, ELECTRODE_HEIGHT], 'LineWidth', line_width, 'Color', [cM(cmapPos(kk), :), line_alpha]);
    tmp_elec_size(displayStruct.chanPairs(kk, :)) = tmp_elec_size(displayStruct.chanPairs(kk, :)) + add_size;
end

for kk = 1:length(tmp_elec_size)
    plot3(y(kk),x(kk),ELECTRODE_HEIGHT,...
        EMARKER,'Color',ECOLOR,'markersize',tmp_elec_size(kk),'linewidth',EMARKERLINEWIDTH);
end
